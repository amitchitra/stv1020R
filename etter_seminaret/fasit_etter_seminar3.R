...
#c)
ssa <- subset(aidgrowth, aidgrowth$sub_saharan_africa==1)
table(ssa$sub_saharan_africa)
#d)
summary(m1 <-lm(gdp_growth ~ aid, data = ssa))

## Positiv effekt av bistand, men ikke signifikant.
...
aidgrowth$assa_eth <- aidgrowth$assasinations*aidgrowth$ethnic_frac

aidgrowth$assa_eth_d <- ifelse(aidgrowth$assa_eth>0, 1, 0)
